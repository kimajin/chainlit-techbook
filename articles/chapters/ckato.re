...
Chainlit には open source data layer の選択肢として次の 4 つがあります。
...
asyncpg は python の asyncio 向けの PostgreSQL データベースクライアントライブラリです。
boto3 は python から S3 EC2, DynamoDB を操作するための AWS 公式 SDK です。
prisma は Node.js や TypeScript 環境で利用される ORM ツールです。
...
tool では 2 秒まってレスポンスを返す処理を一つの step として定義しています。
...
//image[ckato-01][チャット][scale=0.7]{
//}

//image[ckato-02][prisma studio][scale=0.7]{
//}
