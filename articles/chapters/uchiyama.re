...
本稿では、LLMの入出力を手軽にUI上に表示できるという強みを活かして、開発者がLLM/Agentの挙動を観察・デバッグするためのUIフレームワークとして利用する例を紹介します。

LLMアプリケーションの観察に特化したツールはいくつか存在しますが、セキュリティ面の制約などから、その環境を準備するのが難しいこともあります。そのような状況でもまずはクイックにLLMの挙動の観察を始めてみたい、という方には、今回紹介するChainlitを利用した方法が役に立つのではないかと思います。

==== ハンズオンの準備
以降の章のハンズオンでは、実際にChainlitを動かしていきます。

本稿で取り扱うソースコードはサポートページ(https://github.com/y-uty/chainlit-observation)で公開していますので、ハンズオンに取り組みたい方はリポジトリを実行環境にクローンして @<code>{uv sync} で環境を構築してください。
...
=== ハンズオン: LLM呼び出しの観察
実際の観察コードつきChainlitを動かして、LLMに何か質問して結果を確認してみましょう。
...
今回は処理内で時間を計測してレイテンシも計算しています。複雑なMulti-Step Agentがレイテンシの課題を抱えているとき、どのツールがネックになっているかを特定するのに役立ちます。
...
テキストをLLMで要約する機能を考えます。要約自体はLLMを使えば比較的単純なタスクですが、要約処理にかけてよいコストや、どのような要約形式が良いのかは要件ごとに異なるところです。

そこで、以下の4つの要約スタイルをプロンプトとしてLLMに指示し、その要約結果、トークン数・レイテンシを比較してみます。
//emlist[][]{
シンプル: 最小限の指示（ベースライン）
詳細指示: 具体的な要件や制約を含む
...
ここでFew-shotとは、単に「〜〜してください」と指示するのではなく、数件程度の具体例を含んだプロンプトを用いて指示を与えることで、タスクの精度を向上させるプロンプトエンジニアリング手法です。たとえば、以下のようなプロンプトを用意します。
//emlist[Few-shotの例][]{
例1:
...
以下に掲載する例では、「都市交通と自転車」をテーマとした1,700字程度のテキストを要約させています。サンプルコードに含めているので、適当なテキストが無い場合はこちらを使ってください。
